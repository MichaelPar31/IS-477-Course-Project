salary_df_clean = salary_df[['Player', 'Salary', 'Season']]
player_data_df_clean = player_data_df_actual
player_data_df_clean = player_data_df_clean.rename(columns={
    "SEASON_ID": "Season",
    "Name": "Player",
})
player_data_df_clean['Season'] = player_data_df_clean['Season'].str[:4].astype(int)

def normalize_name(name):
    name = str(name).strip().lower()
    return name

player_data_df_clean['Player'] = player_data_df_clean['Player'].apply(normalize_name)
salary_df_clean['Player'] = salary_df_clean['Player'].apply(normalize_name)
...
compare = rl.Compare()
compare.string('Player', 'Player', method='levenshtein', threshold=0.8, label='player_match')
...
stats_unmatched = stats_unmatched.rename(columns={'PTS':'PTS','AST':'AST','REB':'REB','TOV':'TOV','Season':'Season'})
...
salary_unmatched = salary_unmatched[['Player','Season','Salary','PTS','AST','REB','TOV','IntegrationMethod','Source']]
stats_unmatched = stats_unmatched[['Player','Season','Salary','PTS','AST','REB','TOV','IntegrationMethod','Source']]
